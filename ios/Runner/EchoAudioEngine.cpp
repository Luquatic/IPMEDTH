...
aaudio_data_callback_result_t EchoAudioEngine::dataCallback(AAudioStream *stream,
    ...
    if (isEchoOn_) {
        ...
        if (recordingStream_ != nullptr) {
            ...
            returnAudioEffect()->process(static_cast<int16_t *>(audioData), outputChannelCount_, frameCount);
...
void EchoAudioEngine::ConvertMonoToStereo(int16_t *buffer, int32_t numFrames)  {
...
AudioEffect* EchoAudioEngine::returnAudioEffect() {
